...
class build_part_attention_vit(nn.Module):
    def __init__(self, num_classes, cfg, factory, pretrain_tag='imagenet'):
        super().__init__()
        self.cfg = cfg
        self.use_adsl = getattr(cfg.MODEL, "USE", False)
        ...
        self.bottleneck_part1 = nn.BatchNorm1d(self.in_planes)
        self.bottleneck_part1.bias.requires_grad_(False)
        self.bottleneck_part1.apply(weights_init_kaiming)
        self.classifier_part1 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_part1.apply(weights_init_classifier)

        self.bottleneck_part2 = nn.BatchNorm1d(self.in_planes)
        self.bottleneck_part2.bias.requires_grad_(False)
        self.bottleneck_part2.apply(weights_init_kaiming)
        self.classifier_part2 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_part2.apply(weights_init_classifier)

        self.bottleneck_part3 = nn.BatchNorm1d(self.in_planes)
        self.bottleneck_part3.bias.requires_grad_(False)
        self.bottleneck_part3.apply(weights_init_kaiming)
        self.classifier_part3 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_part3.apply(weights_init_classifier)

    def forward(self, x):
        layerwise_tokens = self.base(x)  # list of [B, N, C]
        layerwise_cls_tokens = [t[:, 0] for t in layerwise_tokens]  # 每层的 cls token
        layerwise_part_tokens = [[t[:, i] for i in range(1, 4)] for t in layerwise_tokens]  # 12 层 * 3 个局部 token
        ...
        if not self.use_adsl:
            return cls_score, layerwise_cls_tokens, layerwise_part_tokens

        part1_feat_raw = layerwise_tokens[-1][:, 1]  # [B, C]
        part2_feat_raw = layerwise_tokens[-1][:, 2]
        part3_feat_raw = layerwise_tokens[-1][:, 3]

        part1_feat = self.bottleneck_part1(part1_feat_raw)
        part2_feat = self.bottleneck_part2(part2_feat_raw)
        part3_feat = self.bottleneck_part3(part3_feat_raw)

        part1_score = self.classifier_part1(part1_feat)
        part2_score = self.classifier_part2(part2_feat)
        part3_score = self.classifier_part3(part3_feat)

        part_scores = [part1_score, part2_score, part3_score]
        part_feats = [part1_feat_raw, part2_feat_raw, part3_feat_raw]

        return cls_score, layerwise_cls_tokens, layerwise_part_tokens, part_scores, part_feats
